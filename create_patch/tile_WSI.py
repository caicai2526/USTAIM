...
class DeepZoomImageTiler(object):
    ...
    def _write_tiles(self):
        ...
        print('level',self._dz.level_count-1)

        max_width, max_height = self._dz.level_dimensions[self._dz.level_count-1]
            ...
            for level in range(self._dz.level_count-1,-1,-1):
                ...
                if not os.path.exists(tiledir):
                    os.makedirs(tiledir)
                     ...
                     print(f"Folder '{tiledir}' already exists.")
                     continue
                ...
                for row in range(rows):
                    for col in range(cols):
                        ...
                        if InsertBaseName:
                          tilename = os.path.join(tiledir, '%s_%d_%d_%d_%d_%d_%d.%s' % (
                                          self._basenameJPG, max_width, max_height, (col)*self._dz._z_t_downsample, (row)*self._dz._z_t_downsample, (col+1)*self._dz._z_t_downsample, (row+1)*self._dz._z_t_downsample, self._format))
                          tilename_bw = os.path.join(tiledir, '%s_%d_%d_%d_%d_%d_%d_mask.%s' % (
                                          self._basenameJPG, max_width, max_height, (col)*self._dz._z_t_downsample, (row)*self._dz._z_t_downsample, (col+1)*self._dz._z_t_downsample, (row+1)*self._dz._z_t_downsample, self._format))
                        else:
                          tilename = os.path.join(tiledir, '%s_%d_%d_%d_%d_%d_%d.%s' % (
                                          self._basenameJPG, max_width, max_height, (col)*self._dz._z_t_downsample, (row)*self._dz._z_t_downsample, (col+1)*self._dz._z_t_downsample, (row+1)*self._dz._z_t_downsample, self._format))
...
if __name__ == '__main__':
	...
	parser.add_option('-o', '--output', metavar='NAME', dest='basename',
		help='base name of output file',default='"E:\\Project_Result\\diagnosis_UC_CD\\processing\\Graph_Transformer_patch256\\Normal\\')
	...
	# # Initialization
	# slidepath = "E:\\Normal_colon\\Normal/*.svs"
	# tile_size = 512
	...
	for imgNb in range(len(files)):
		...
		# elif opts.xmlfile != '': # opts.xmlfile != '':
				...
				if (opts.mask_type==1) or (opts.oLabelref!=''):
					...
					for oLabel in xml_labels:
						...
						if Nbr_ROIs_ForNegLabel==0:
							...
							# a label ref was given, and incverse mask is required but no ROI with this label in that map --> take everything
